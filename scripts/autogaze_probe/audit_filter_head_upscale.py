"""r/grid-upscale-frozen: 20-pair qual audit + bench for the attn-pool ImageLevelFilterHead
trained on bilinear-UPSCALED L3 features (14x14 -> 28x28), vs IconStudent-118K.

Same audit framework as filter-head-direct-qual; only difference is that the AutoGaze
L3 hidden states are bilinear-upscaled to TARGET_HW**2 patches before being fed to the
head (and the per-patch heatmap is output at TARGET_HW**2 resolution before the final
...
def load_filter_head(ckpt: str, device):
    ...
    head.eval()
    target_hw = int(raw.get("target_hw", 28))
    return head, target_hw
...
@torch.inference_mode()
def predict_filter_head(image_pil, query, autogaze_wrapper, head, target_hw, device, h, w):
    """End-to-end: 224x224 squash -> AutoGaze L3 (14x14) -> bilinear upscale to target_hw -> filter head -> per-patch sigmoid -> upsample."""
    ...
    hidden_up = upscale_patches(hidden, target_hw)  # (1, target_hw**2, 192)
    logits = head.patch_logits(hidden_up, query.unsqueeze(0).to(device))  # (1, 1, target_hw**2)
    probs = torch.sigmoid(logits)[0, 0].view(target_hw, target_hw).cpu().numpy()
    sq = F.interpolate(
...
@torch.inference_mode()
def benchmark(autogaze_wrapper, head, target_hw, image_pil, query, device,
    ...
    for _ in range(n_warmup):
        ...
        h_up = upscale_patches(h_, target_hw)
        _ = head.patch_logits(h_up, q)
    ag_times, head_times, total_times = [], [], []
    for _ in range(n_runs):
        t0 = time.perf_counter()
        hidden = autogaze_wrapper.get_patch_hidden_states(pre_t)
        t1 = time.perf_counter()
        hidden_up = upscale_patches(hidden, target_hw)
        logits = head.patch_logits(hidden_up, q)
...
def main():
    ...
    p.add_argument("--head-ckpt", default="results/filter_head_upscale/best.pt")
    ...
    p.add_argument("--output-dir", default="results/filter_head_upscale/audit")
    p.add_argument("--bench-out", default="results/filter_head_upscale/bench.json")
    p.add_argument("--summary-out", default="results/filter_head_upscale/summary.json")
    ...
    head, target_hw = load_filter_head(args.head_ckpt, device)
    print(f"[diag] target_hw = {target_hw} (N={target_hw*target_hw} patches)")
    ...
    for img_id, cat_slug in bench_pairs:
        ...
        b = benchmark(w_model, head, target_hw, img, query, device,
    ...
    for idx, (img_id, cat_slug) in enumerate(pairs):
        ...
        p_filter = predict_filter_head(img, query, w_model, head, target_hw, device, h, w_im)
