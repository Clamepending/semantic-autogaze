"""r/filter-head-direct-qual: 20-pair qual audit + bench for the cycle-2 attn-pool
ImageLevelFilterHead's per-patch heatmap, vs IconStudent-118K.

Produces 4-panel figures: [image | COCO GT | AutoGaze+filter_head per-patch | IconStudent-118K].
Reports per-pair IoU (heatmap > 0.5 vs GT) for both methods, and tallies W/L/T from IoU
diff with a small tie band (|ΔIoU| < 0.05). Writes summary.json so the W/L/T count
is reproducible.

Re-uses scripts/autogaze_probe/bench_and_audit.py preprocessing for AutoGaze + IconStudent.
"""
...
@torch.inference_mode()
def predict_filter_head(image_pil, query, autogaze_wrapper, head, device, h, w):
    """End-to-end: 224x224 squash -> AutoGaze L3 -> filter head -> per-patch sigmoid -> upsample."""
...
def main():
    ...
    p.add_argument("--head-ckpt", default="results/filter_head_retrain/best_attn.pt")
    ...
    p.add_argument("--output-dir", default="results/filter_head_direct_qual/audit")
    p.add_argument("--bench-out", default="results/filter_head_direct_qual/bench.json")
    p.add_argument("--summary-out", default="results/filter_head_direct_qual/summary.json")
    ...
    head = load_filter_head(args.head_ckpt, device)
    ...
    for idx, (img_id, cat_slug) in enumerate(pairs):
        ...
        p_filter = predict_filter_head(img, query, w_model, head, device, h, w_im)
